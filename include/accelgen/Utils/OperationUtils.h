...
class Array2D {
 private:
  ...
  llvm::SmallVector<uint64_t, 2> _bounds;

 public:
  Array2D(llvm::ArrayRef<uint64_t> bounds)
      : _bounds(bounds.begin(), bounds.end()) {
    assert(_bounds.size() == 2);
    _dat = llvm::SmallVector<_Type>(_bounds[0] * _bounds[1]);
  }
  Array2D(llvm::ArrayRef<uint64_t> bounds, llvm::ArrayRef<_Type> dat)
      : _bounds(bounds.begin(), bounds.end()) {
    assert(_bounds.size() == 2);
  ...
  llvm::SmallVector<_Type> col(uint64_t idx) {
    llvm::SmallVector<_Type> vec;
    for (uint64_t i = 0; i < _bounds[0]; i++) {
      vec.push_back(at(i, idx));
    }
    return vec;
  }

  llvm::SmallVector<_Type> row(uint64_t idx) {
    llvm::SmallVector<_Type> vec;
    for (uint64_t i = 0; i < _bounds[1]; i++) {
      vec.push_back(at(idx, i));
    }
    return vec;
  }
  ...
  size_t getNumRow() { return _bounds[0]; }
  size_t getNumCol() { return _bounds[1]; }
};
