...
LOG_DIR = os.path.join(WORK_DIR, "baseline_test/logs")
TESTORDER_FILE = os.path.join(WORK_DIR, "baseline_test/testorder.txt")
SUMMARY_DIR = os.path.join(WORK_DIR, "baseline_test")
...
def parse_log(log_path):
    ...
    m = re.search(r"Datamove total: Cycles=(\d+)\s+MemEnergy=([\d.eE+\-]+)", content)
    if m:
    ...
    for i, chunk in enumerate(chunks):
        ...
        with open(script_path, "w") as f:
            ...
            for j, cmd in enumerate(chunk):
                ...
                f.write(f"{cmd} 2>&1 | tee {log_file}\n")
