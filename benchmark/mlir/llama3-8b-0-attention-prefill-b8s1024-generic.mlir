...
module {
  func.func @main(%arg0: tensor<8x1024x4096xbf16>, %arg1: tensor<1x1024x128xbf16>, %arg2: tensor<1x1024x128xbf16>, %arg3: tensor<1x32x1x1xbf16>, %arg4: tensor<4096x4096xbf16>, %arg5: tensor<4096x1024xbf16>, %arg6: tensor<4096x1024xbf16>, %arg7: tensor<4096x4096xbf16>) -> (tensor<8x1024x4096xbf16>, tensor<8x32x1024x1024xbf16>) {
    ...
    ^bb0(%in: bf16, %out: bf16):
      %53 = arith.truncf %cst_2 : f64 to bf16
      %54 = arith.mulf %in, %53 : bf16
      linalg.yield %54 : bf16
    ...
    %35 = tensor.empty() : tensor<8x32x1024xi64>
    %36 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%c0_i64 : i64) outs(%35 : tensor<8x32x1024xi64>) attrs =  {accelgen.const_broadcast = true, accelgen.memory_transformation = true} {
    ^bb0(%in: i64, %out: i64):
      linalg.yield %in : i64
    } -> tensor<8x32x1024xi64>
    %37 = tensor.empty() : tensor<8x32x1024xf32>
    %38 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%cst_0 : f32) outs(%37 : tensor<8x32x1024xf32>) attrs =  {accelgen.const_broadcast = true, accelgen.memory_transformation = true} {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<8x32x1024xf32>
    %39:2 = linalg.generic {indexing_maps = [#map6, #map4, #map4], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%34 : tensor<8x32x1024x1024xf32>) outs(%38, %36 : tensor<8x32x1024xf32>, tensor<8x32x1024xi64>) {
    ^bb0(%in: f32, %out: f32, %out_19: i64):
      ...
      %57 = arith.select %56, %54, %out_19 : i64
      linalg.yield %55, %57 : f32, i64
    } -> (tensor<8x32x1024xf32>, tensor<8x32x1024xi64>)
    %expanded_14 = tensor.expand_shape %39#0 [[0], [1], [2, 3]] output_shape [8, 32, 1024, 1] : tensor<8x32x1024xf32> into tensor<8x32x1024x1xf32>
    %40 = linalg.generic {indexing_maps = [#map6, #map13, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%34, %expanded_14 : tensor<8x32x1024x1024xf32>, tensor<8x32x1024x1xf32>) outs(%33 : tensor<8x32x1024x1024xf32>) {
    ^bb0(%in: f32, %in_19: f32, %out: f32):
      %53 = arith.subf %in, %in_19 : f32
      linalg.yield %53 : f32
    } -> tensor<8x32x1024x1024xf32>
    %41 = linalg.generic {indexing_maps = [#map6, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%40 : tensor<8x32x1024x1024xf32>) outs(%33 : tensor<8x32x1024x1024xf32>) {
    ^bb0(%in: f32, %out: f32):
      %53 = math.exp %in : f32
      linalg.yield %53 : f32
    } -> tensor<8x32x1024x1024xf32>
    %42 = tensor.empty() : tensor<8x32x1024x1xf32>
    %43 = linalg.generic {indexing_maps = [#map14, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%cst_1 : f32) outs(%42 : tensor<8x32x1024x1xf32>) attrs =  {accelgen.const_broadcast = true, accelgen.memory_transformation = true} {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<8x32x1024x1xf32>
    %44 = linalg.generic {indexing_maps = [#map6, #map13], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%41 : tensor<8x32x1024x1024xf32>) outs(%43 : tensor<8x32x1024x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %53 = arith.addf %in, %out : f32
      linalg.yield %53 : f32
    } -> tensor<8x32x1024x1xf32>
    %45 = linalg.generic {indexing_maps = [#map6, #map13, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%41, %44 : tensor<8x32x1024x1024xf32>, tensor<8x32x1024x1xf32>) outs(%33 : tensor<8x32x1024x1024xf32>) {
    ^bb0(%in: f32, %in_19: f32, %out: f32):
      %53 = arith.divf %in, %in_19 : f32
      linalg.yield %53 : f32
    } -> tensor<8x32x1024x1024xf32>
    %46 = linalg.generic {indexing_maps = [#map6, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%45 : tensor<8x32x1024x1024xf32>) outs(%30 : tensor<8x32x1024x1024xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %53 = arith.truncf %in : f32 to bf16
      linalg.yield %53 : bf16
    } -> tensor<8x32x1024x1024xbf16>
    %collapsed_15 = tensor.collapse_shape %46 [[0, 1], [2], [3]] : tensor<8x32x1024x1024xbf16> into tensor<256x1024x1024xbf16>
    %collapsed_16 = tensor.collapse_shape %24 [[0, 1, 2], [3], [4]] : tensor<8x8x4x1024x128xbf16> into tensor<256x1024x128xbf16>
    %47 = tensor.empty() : tensor<256x1024x128xbf16>
    %48 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%cst : bf16) outs(%47 : tensor<256x1024x128xbf16>) attrs =  {accelgen.const_broadcast = true, accelgen.memory_transformation = true} {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<256x1024x128xbf16>
    %49 = linalg.generic {indexing_maps = [#map2, #map11, #map4], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%collapsed_15, %collapsed_16 : tensor<256x1024x1024xbf16>, tensor<256x1024x128xbf16>) outs(%48 : tensor<256x1024x128xbf16>) {
    ^bb0(%in: bf16, %in_19: bf16, %out: bf16):
      %53 = arith.mulf %in, %in_19 : bf16
      %54 = arith.addf %out, %53 : bf16
      linalg.yield %54 : bf16
    } -> tensor<256x1024x128xbf16>
    %expanded_17 = tensor.expand_shape %49 [[0, 1], [2], [3]] output_shape [8, 32, 1024, 128] : tensor<256x1024x128xbf16> into tensor<8x32x1024x128xbf16>
    %50 = tensor.empty() : tensor<8x1024x32x128xbf16>
    %51 = linalg.generic {indexing_maps = [#map5, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_17 : tensor<8x32x1024x128xbf16>) outs(%50 : tensor<8x1024x32x128xbf16>) attrs =  {accelgen.memory_transformation = true, accelgen.transpose = true} {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x1024x32x128xbf16>
    %collapsed_18 = tensor.collapse_shape %51 [[0], [1], [2, 3]] : tensor<8x1024x32x128xbf16> into tensor<8x1024x4096xbf16>
    %52 = linalg.generic {indexing_maps = [#map2, #map3, #map4], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%collapsed_18, %arg7 : tensor<8x1024x4096xbf16>, tensor<4096x4096xbf16>) outs(%1 : tensor<8x1024x4096xbf16>) {
    ^bb0(%in: bf16, %in_19: bf16, %out: bf16):
      %53 = arith.mulf %in, %in_19 : bf16
      %54 = arith.addf %out, %53 : bf16
      linalg.yield %54 : bf16
    } -> tensor<8x1024x4096xbf16>
    return %52, %46 : tensor<8x1024x4096xbf16>, tensor<8x32x1024x1024xbf16>
  }
}
