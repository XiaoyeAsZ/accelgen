...
module {
  func.func @main(%arg0: tensor<8x1x4096xbf16>, %arg1: tensor<1x1x128xbf16>, %arg2: tensor<1x1x128xbf16>, %arg3: tensor<1x64x1x1024xbf16>, %arg4: tensor<8x4x1023x128xbf16>, %arg5: tensor<8x4x1023x128xbf16>, %arg6: tensor<8192x4096xbf16>, %arg7: tensor<512x4096xbf16>, %arg8: tensor<512x4096xbf16>, %arg9: tensor<4096x8192xbf16>, %arg10: tensor<128xbf16>, %arg11: tensor<128xbf16>) -> tensor<8x1x4096xbf16> {
    ...
    %concat_18 = tensor.concat dim(2) %arg4, %53 : (tensor<8x4x1023x128xbf16>, tensor<8x4x1x128xbf16>) -> tensor<8x4x1024x128xbf16>
    %concat_19 = tensor.concat dim(2) %arg5, %transposed_11 : (tensor<8x4x1023x128xbf16>, tensor<8x4x1x128xbf16>) -> tensor<8x4x1024x128xbf16>
    %54 = tensor.empty() : tensor<8x4x16x1024x128xbf16>
    %55 = linalg.generic {indexing_maps = [#map7, #map8], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%concat_18 : tensor<8x4x1024x128xbf16>) outs(%54 : tensor<8x4x16x1024x128xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x4x16x1024x128xbf16>
    %collapsed_20 = tensor.collapse_shape %55 [[0], [1, 2], [3], [4]] : tensor<8x4x16x1024x128xbf16> into tensor<8x64x1024x128xbf16>
    %56 = linalg.generic {indexing_maps = [#map7, #map8], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%concat_19 : tensor<8x4x1024x128xbf16>) outs(%54 : tensor<8x4x16x1024x128xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x4x16x1024x128xbf16>
    %57 = tensor.empty() : tensor<8x64x128x1024xbf16>
    %transposed_21 = linalg.transpose ins(%collapsed_20 : tensor<8x64x1024x128xbf16>) outs(%57 : tensor<8x64x128x1024xbf16>) permutation = [0, 1, 3, 2] 
    ...
    %collapsed_24 = tensor.collapse_shape %transposed_21 [[0, 1], [2], [3]] : tensor<8x64x128x1024xbf16> into tensor<512x128x1024xbf16>
    %59 = tensor.empty() : tensor<512x1x1024xbf16>
    %60 = linalg.fill ins(%cst : bf16) outs(%59 : tensor<512x1x1024xbf16>) -> tensor<512x1x1024xbf16>
    %61 = linalg.batch_matmul ins(%collapsed_23, %collapsed_24 : tensor<512x1x128xbf16>, tensor<512x128x1024xbf16>) outs(%60 : tensor<512x1x1024xbf16>) -> tensor<512x1x1024xbf16>
    %expanded_25 = tensor.expand_shape %61 [[0, 1], [2], [3]] output_shape [8, 64, 1, 1024] : tensor<512x1x1024xbf16> into tensor<8x64x1x1024xbf16>
    %62 = tensor.empty() : tensor<8x64x1x1024xbf16>
    %63 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_25 : tensor<8x64x1x1024xbf16>) outs(%62 : tensor<8x64x1x1024xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %89 = arith.truncf %cst_2 : f64 to bf16
      %90 = arith.mulf %in, %89 : bf16
      linalg.yield %90 : bf16
    } -> tensor<8x64x1x1024xbf16>
    %64 = linalg.generic {indexing_maps = [#map3, #map10, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%63, %arg3 : tensor<8x64x1x1024xbf16>, tensor<1x64x1x1024xbf16>) outs(%62 : tensor<8x64x1x1024xbf16>) {
    ^bb0(%in: bf16, %in_34: bf16, %out: bf16):
      %89 = arith.addf %in, %in_34 : bf16
      linalg.yield %89 : bf16
    } -> tensor<8x64x1x1024xbf16>
    %65 = tensor.empty() : tensor<8x64x1x1024xf32>
    %66 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%64 : tensor<8x64x1x1024xbf16>) outs(%65 : tensor<8x64x1x1024xf32>) {
    ^bb0(%in: bf16, %out: f32):
      %89 = arith.extf %in : bf16 to f32
      linalg.yield %89 : f32
    ...
    %71:2 = linalg.generic {indexing_maps = [#map3, #map11, #map11], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%66 : tensor<8x64x1x1024xf32>) outs(%70, %68 : tensor<8x64x1xf32>, tensor<8x64x1xi64>) {
    ...
    %72 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%66, %expanded_26 : tensor<8x64x1x1024xf32>, tensor<8x64x1x1xf32>) outs(%65 : tensor<8x64x1x1024xf32>) {
    ^bb0(%in: f32, %in_34: f32, %out: f32):
      %89 = arith.subf %in, %in_34 : f32
      linalg.yield %89 : f32
    } -> tensor<8x64x1x1024xf32>
    %73 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%72 : tensor<8x64x1x1024xf32>) outs(%65 : tensor<8x64x1x1024xf32>) {
    ^bb0(%in: f32, %out: f32):
      %89 = math.exp %in : f32
      linalg.yield %89 : f32
    } -> tensor<8x64x1x1024xf32>
    ...
    %76 = linalg.generic {indexing_maps = [#map3, #map4], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%73 : tensor<8x64x1x1024xf32>) outs(%75 : tensor<8x64x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %89 = arith.addf %in, %out : f32
      linalg.yield %89 : f32
    } -> tensor<8x64x1x1xf32>
    %77 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%73, %76 : tensor<8x64x1x1024xf32>, tensor<8x64x1x1xf32>) outs(%65 : tensor<8x64x1x1024xf32>) {
    ^bb0(%in: f32, %in_34: f32, %out: f32):
      %89 = arith.divf %in, %in_34 : f32
      linalg.yield %89 : f32
    } -> tensor<8x64x1x1024xf32>
    %78 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%77 : tensor<8x64x1x1024xf32>) outs(%62 : tensor<8x64x1x1024xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %89 = arith.truncf %in : f32 to bf16
      linalg.yield %89 : bf16
    } -> tensor<8x64x1x1024xbf16>
    %collapsed_27 = tensor.collapse_shape %78 [[0], [1, 2], [3]] : tensor<8x64x1x1024xbf16> into tensor<8x64x1024xbf16>
    %79 = linalg.generic {indexing_maps = [#map9, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_27 : tensor<8x64x1024xbf16>) outs(%62 : tensor<8x64x1x1024xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x64x1x1024xbf16>
    %collapsed_28 = tensor.collapse_shape %79 [[0, 1], [2], [3]] : tensor<8x64x1x1024xbf16> into tensor<512x1x1024xbf16>
    %collapsed_29 = tensor.collapse_shape %56 [[0, 1, 2], [3], [4]] : tensor<8x4x16x1024x128xbf16> into tensor<512x1024x128xbf16>
    %80 = tensor.empty() : tensor<512x1x128xbf16>
    %81 = linalg.fill ins(%cst : bf16) outs(%80 : tensor<512x1x128xbf16>) -> tensor<512x1x128xbf16>
    %82 = linalg.batch_matmul ins(%collapsed_28, %collapsed_29 : tensor<512x1x1024xbf16>, tensor<512x1024x128xbf16>) outs(%81 : tensor<512x1x128xbf16>) -> tensor<512x1x128xbf16>
