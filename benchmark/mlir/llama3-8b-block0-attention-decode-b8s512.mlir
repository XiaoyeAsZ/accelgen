...
module {
  func.func @main(%arg0: tensor<8x1x4096xbf16>, %arg1: tensor<1x1x128xbf16>, %arg2: tensor<1x1x128xbf16>, %arg3: tensor<1x32x1x1xbf16>, %arg4: tensor<8x8x511x128xbf16>, %arg5: tensor<8x8x511x128xbf16>, %arg6: tensor<4096x4096xbf16>, %arg7: tensor<1024x4096xbf16>, %arg8: tensor<1024x4096xbf16>, %arg9: tensor<4096x4096xbf16>) -> (tensor<8x1x4096xbf16>, tensor<8x32x1x512xbf16>) {
    ...
    %27 = tensor.empty() : tensor<8x8x4x512x128xbf16>
    %28 = linalg.generic {indexing_maps = [#map5, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%concat_16 : tensor<8x8x512x128xbf16>) outs(%27 : tensor<8x8x4x512x128xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x8x4x512x128xbf16>
    %collapsed_18 = tensor.collapse_shape %28 [[0], [1, 2], [3], [4]] : tensor<8x8x4x512x128xbf16> into tensor<8x32x512x128xbf16>
    %29 = linalg.generic {indexing_maps = [#map5, #map6], iterator_types = ["parallel", "parallel", "parallel", "parallel", "parallel"]} ins(%concat_17 : tensor<8x8x512x128xbf16>) outs(%27 : tensor<8x8x4x512x128xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x8x4x512x128xbf16>
    %30 = tensor.empty() : tensor<8x32x128x512xbf16>
    %transposed_19 = linalg.transpose ins(%collapsed_18 : tensor<8x32x512x128xbf16>) outs(%30 : tensor<8x32x128x512xbf16>) permutation = [0, 1, 3, 2] 
    ...
    %collapsed_22 = tensor.collapse_shape %transposed_19 [[0, 1], [2], [3]] : tensor<8x32x128x512xbf16> into tensor<256x128x512xbf16>
    %32 = tensor.empty() : tensor<256x1x512xbf16>
    %33 = linalg.fill ins(%cst : bf16) outs(%32 : tensor<256x1x512xbf16>) -> tensor<256x1x512xbf16>
    %34 = linalg.batch_matmul ins(%collapsed_21, %collapsed_22 : tensor<256x1x128xbf16>, tensor<256x128x512xbf16>) outs(%33 : tensor<256x1x512xbf16>) -> tensor<256x1x512xbf16>
    %expanded_23 = tensor.expand_shape %34 [[0, 1], [2], [3]] output_shape [8, 32, 1, 512] : tensor<256x1x512xbf16> into tensor<8x32x1x512xbf16>
    %35 = tensor.empty() : tensor<8x32x1x512xbf16>
    %36 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_23 : tensor<8x32x1x512xbf16>) outs(%35 : tensor<8x32x1x512xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %60 = arith.truncf %cst_2 : f64 to bf16
      %61 = arith.mulf %in, %60 : bf16
      linalg.yield %61 : bf16
    } -> tensor<8x32x1x512xbf16>
    %37 = linalg.generic {indexing_maps = [#map3, #map8, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%36, %arg3 : tensor<8x32x1x512xbf16>, tensor<1x32x1x1xbf16>) outs(%35 : tensor<8x32x1x512xbf16>) {
    ^bb0(%in: bf16, %in_32: bf16, %out: bf16):
      %60 = arith.addf %in, %in_32 : bf16
      linalg.yield %60 : bf16
    } -> tensor<8x32x1x512xbf16>
    %38 = tensor.empty() : tensor<8x32x1x512xf32>
    %39 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%37 : tensor<8x32x1x512xbf16>) outs(%38 : tensor<8x32x1x512xf32>) {
    ^bb0(%in: bf16, %out: f32):
    ...
    %44:2 = linalg.generic {indexing_maps = [#map3, #map9, #map9], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%39 : tensor<8x32x1x512xf32>) outs(%43, %41 : tensor<8x32x1xf32>, tensor<8x32x1xi64>) {
    ...
    %45 = linalg.generic {indexing_maps = [#map3, #map10, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%39, %expanded_24 : tensor<8x32x1x512xf32>, tensor<8x32x1x1xf32>) outs(%38 : tensor<8x32x1x512xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %60 = arith.subf %in, %in_32 : f32
      linalg.yield %60 : f32
    } -> tensor<8x32x1x512xf32>
    %46 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%45 : tensor<8x32x1x512xf32>) outs(%38 : tensor<8x32x1x512xf32>) {
    ^bb0(%in: f32, %out: f32):
      %60 = math.exp %in : f32
      linalg.yield %60 : f32
    } -> tensor<8x32x1x512xf32>
    ...
    %49 = linalg.generic {indexing_maps = [#map3, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%46 : tensor<8x32x1x512xf32>) outs(%48 : tensor<8x32x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %60 = arith.addf %in, %out : f32
      linalg.yield %60 : f32
    } -> tensor<8x32x1x1xf32>
    %50 = linalg.generic {indexing_maps = [#map3, #map10, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%46, %49 : tensor<8x32x1x512xf32>, tensor<8x32x1x1xf32>) outs(%38 : tensor<8x32x1x512xf32>) {
    ^bb0(%in: f32, %in_32: f32, %out: f32):
      %60 = arith.divf %in, %in_32 : f32
      linalg.yield %60 : f32
    } -> tensor<8x32x1x512xf32>
    %51 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%50 : tensor<8x32x1x512xf32>) outs(%35 : tensor<8x32x1x512xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %60 = arith.truncf %in : f32 to bf16
      linalg.yield %60 : bf16
    } -> tensor<8x32x1x512xbf16>
    %collapsed_25 = tensor.collapse_shape %51 [[0], [1, 2], [3]] : tensor<8x32x1x512xbf16> into tensor<8x32x512xbf16>
    %52 = linalg.generic {indexing_maps = [#map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_25 : tensor<8x32x512xbf16>) outs(%35 : tensor<8x32x1x512xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x32x1x512xbf16>
    %collapsed_26 = tensor.collapse_shape %52 [[0, 1], [2], [3]] : tensor<8x32x1x512xbf16> into tensor<256x1x512xbf16>
    %collapsed_27 = tensor.collapse_shape %29 [[0, 1, 2], [3], [4]] : tensor<8x8x4x512x128xbf16> into tensor<256x512x128xbf16>
    %53 = tensor.empty() : tensor<256x1x128xbf16>
    %54 = linalg.fill ins(%cst : bf16) outs(%53 : tensor<256x1x128xbf16>) -> tensor<256x1x128xbf16>
    %55 = linalg.batch_matmul ins(%collapsed_26, %collapsed_27 : tensor<256x1x512xbf16>, tensor<256x512x128xbf16>) outs(%54 : tensor<256x1x128xbf16>) -> tensor<256x1x128xbf16>
    ...
    return %59, %51 : tensor<8x1x4096xbf16>, tensor<8x32x1x512xbf16>
  }
}
