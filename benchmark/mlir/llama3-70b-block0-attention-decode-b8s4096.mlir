...
module {
  func.func @main(%arg0: tensor<8x1x4096xbf16>, %arg1: tensor<1x1x128xbf16>, %arg2: tensor<1x1x128xbf16>, %arg3: tensor<1x32x1x1xbf16>, %arg4: tensor<8x32x4095x128xbf16>, %arg5: tensor<8x32x4095x128xbf16>, %arg6: tensor<4096x4096xbf16>, %arg7: tensor<4096x4096xbf16>, %arg8: tensor<4096x4096xbf16>, %arg9: tensor<4096x4096xbf16>) -> (tensor<8x1x4096xbf16>, tensor<8x32x1x4096xbf16>) {
    ...
    %concat_16 = tensor.concat dim(2) %arg4, %20 : (tensor<8x32x4095x128xbf16>, tensor<8x32x1x128xbf16>) -> tensor<8x32x4096x128xbf16>
    %concat_17 = tensor.concat dim(2) %arg5, %transposed_9 : (tensor<8x32x4095x128xbf16>, tensor<8x32x1x128xbf16>) -> tensor<8x32x4096x128xbf16>
    %21 = tensor.empty() : tensor<8x32x128x4096xbf16>
    %transposed_18 = linalg.transpose ins(%concat_16 : tensor<8x32x4096x128xbf16>) outs(%21 : tensor<8x32x128x4096xbf16>) permutation = [0, 1, 3, 2] 
    ...
    %collapsed_21 = tensor.collapse_shape %transposed_18 [[0, 1], [2], [3]] : tensor<8x32x128x4096xbf16> into tensor<256x128x4096xbf16>
    %23 = tensor.empty() : tensor<256x1x4096xbf16>
    %24 = linalg.fill ins(%cst : bf16) outs(%23 : tensor<256x1x4096xbf16>) -> tensor<256x1x4096xbf16>
    %25 = linalg.batch_matmul ins(%collapsed_20, %collapsed_21 : tensor<256x1x128xbf16>, tensor<256x128x4096xbf16>) outs(%24 : tensor<256x1x4096xbf16>) -> tensor<256x1x4096xbf16>
    %expanded_22 = tensor.expand_shape %25 [[0, 1], [2], [3]] output_shape [8, 32, 1, 4096] : tensor<256x1x4096xbf16> into tensor<8x32x1x4096xbf16>
    %26 = tensor.empty() : tensor<8x32x1x4096xbf16>
    %27 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_22 : tensor<8x32x1x4096xbf16>) outs(%26 : tensor<8x32x1x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %51 = arith.truncf %cst_2 : f64 to bf16
      %52 = arith.mulf %in, %51 : bf16
      linalg.yield %52 : bf16
    } -> tensor<8x32x1x4096xbf16>
    %28 = linalg.generic {indexing_maps = [#map3, #map6, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%27, %arg3 : tensor<8x32x1x4096xbf16>, tensor<1x32x1x1xbf16>) outs(%26 : tensor<8x32x1x4096xbf16>) {
    ^bb0(%in: bf16, %in_31: bf16, %out: bf16):
      %51 = arith.addf %in, %in_31 : bf16
      linalg.yield %51 : bf16
    } -> tensor<8x32x1x4096xbf16>
    %29 = tensor.empty() : tensor<8x32x1x4096xf32>
    %30 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%28 : tensor<8x32x1x4096xbf16>) outs(%29 : tensor<8x32x1x4096xf32>) {
    ^bb0(%in: bf16, %out: f32):
    ...
    %35:2 = linalg.generic {indexing_maps = [#map3, #map7, #map7], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%30 : tensor<8x32x1x4096xf32>) outs(%34, %32 : tensor<8x32x1xf32>, tensor<8x32x1xi64>) {
    ...
    %36 = linalg.generic {indexing_maps = [#map3, #map8, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%30, %expanded_23 : tensor<8x32x1x4096xf32>, tensor<8x32x1x1xf32>) outs(%29 : tensor<8x32x1x4096xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %51 = arith.subf %in, %in_31 : f32
      linalg.yield %51 : f32
    } -> tensor<8x32x1x4096xf32>
    %37 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%36 : tensor<8x32x1x4096xf32>) outs(%29 : tensor<8x32x1x4096xf32>) {
    ^bb0(%in: f32, %out: f32):
      %51 = math.exp %in : f32
      linalg.yield %51 : f32
    } -> tensor<8x32x1x4096xf32>
    ...
    %40 = linalg.generic {indexing_maps = [#map3, #map8], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%37 : tensor<8x32x1x4096xf32>) outs(%39 : tensor<8x32x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %51 = arith.addf %in, %out : f32
      linalg.yield %51 : f32
    } -> tensor<8x32x1x1xf32>
    %41 = linalg.generic {indexing_maps = [#map3, #map8, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%37, %40 : tensor<8x32x1x4096xf32>, tensor<8x32x1x1xf32>) outs(%29 : tensor<8x32x1x4096xf32>) {
    ^bb0(%in: f32, %in_31: f32, %out: f32):
      %51 = arith.divf %in, %in_31 : f32
      linalg.yield %51 : f32
    } -> tensor<8x32x1x4096xf32>
    %42 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%41 : tensor<8x32x1x4096xf32>) outs(%26 : tensor<8x32x1x4096xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %51 = arith.truncf %in : f32 to bf16
      linalg.yield %51 : bf16
    } -> tensor<8x32x1x4096xbf16>
    %collapsed_24 = tensor.collapse_shape %42 [[0], [1, 2], [3]] : tensor<8x32x1x4096xbf16> into tensor<8x32x4096xbf16>
    %43 = linalg.generic {indexing_maps = [#map5, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_24 : tensor<8x32x4096xbf16>) outs(%26 : tensor<8x32x1x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<8x32x1x4096xbf16>
    %collapsed_25 = tensor.collapse_shape %43 [[0, 1], [2], [3]] : tensor<8x32x1x4096xbf16> into tensor<256x1x4096xbf16>
    %collapsed_26 = tensor.collapse_shape %concat_17 [[0, 1], [2], [3]] : tensor<8x32x4096x128xbf16> into tensor<256x4096x128xbf16>
    %44 = tensor.empty() : tensor<256x1x128xbf16>
    %45 = linalg.fill ins(%cst : bf16) outs(%44 : tensor<256x1x128xbf16>) -> tensor<256x1x128xbf16>
    %46 = linalg.batch_matmul ins(%collapsed_25, %collapsed_26 : tensor<256x1x4096xbf16>, tensor<256x4096x128xbf16>) outs(%45 : tensor<256x1x128xbf16>) -> tensor<256x1x128xbf16>
    ...
    return %50, %42 : tensor<8x1x4096xbf16>, tensor<8x32x1x4096xbf16>
  }
}
