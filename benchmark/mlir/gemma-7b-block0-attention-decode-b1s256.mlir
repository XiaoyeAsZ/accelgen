...
module {
  func.func @main(%arg0: tensor<1x1x3072xbf16>, %arg1: tensor<1x1x256xbf16>, %arg2: tensor<1x1x256xbf16>, %arg3: tensor<1x16x1x1xbf16>, %arg4: tensor<1x16x255x256xbf16>, %arg5: tensor<1x16x255x256xbf16>, %arg6: tensor<4096x3072xbf16>, %arg7: tensor<4096x3072xbf16>, %arg8: tensor<4096x3072xbf16>, %arg9: tensor<3072x4096xbf16>) -> tensor<1x1x3072xbf16> {
    ...
    %concat_16 = tensor.concat dim(2) %arg4, %21 : (tensor<1x16x255x256xbf16>, tensor<1x16x1x256xbf16>) -> tensor<1x16x256x256xbf16>
    %concat_17 = tensor.concat dim(2) %arg5, %transposed_9 : (tensor<1x16x255x256xbf16>, tensor<1x16x1x256xbf16>) -> tensor<1x16x256x256xbf16>
    %22 = tensor.empty() : tensor<1x16x256x256xbf16>
    %transposed_18 = linalg.transpose ins(%concat_16 : tensor<1x16x256x256xbf16>) outs(%22 : tensor<1x16x256x256xbf16>) permutation = [0, 1, 3, 2] 
    ...
    %collapsed_20 = tensor.collapse_shape %transposed_18 [[0, 1], [2], [3]] : tensor<1x16x256x256xbf16> into tensor<16x256x256xbf16>
    %24 = linalg.generic {indexing_maps = [#map6, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_20 : tensor<16x256x256xbf16>) outs(%22 : tensor<1x16x256x256xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x256x256xbf16>
    %collapsed_21 = tensor.collapse_shape %23 [[0, 1], [2], [3]] : tensor<1x16x1x256xbf16> into tensor<16x1x256xbf16>
    %collapsed_22 = tensor.collapse_shape %24 [[0, 1], [2], [3]] : tensor<1x16x256x256xbf16> into tensor<16x256x256xbf16>
    %25 = tensor.empty() : tensor<16x1x256xbf16>
    %26 = linalg.fill ins(%cst : bf16) outs(%25 : tensor<16x1x256xbf16>) -> tensor<16x1x256xbf16>
    %27 = linalg.batch_matmul ins(%collapsed_21, %collapsed_22 : tensor<16x1x256xbf16>, tensor<16x256x256xbf16>) outs(%26 : tensor<16x1x256xbf16>) -> tensor<16x1x256xbf16>
    %expanded_23 = tensor.expand_shape %27 [[0, 1], [2], [3]] output_shape [1, 16, 1, 256] : tensor<16x1x256xbf16> into tensor<1x16x1x256xbf16>
    %28 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_23 : tensor<1x16x1x256xbf16>) outs(%8 : tensor<1x16x1x256xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %54 = arith.mulf %in, %cst_2 : bf16
      linalg.yield %54 : bf16
    } -> tensor<1x16x1x256xbf16>
    %29 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%28, %arg3 : tensor<1x16x1x256xbf16>, tensor<1x16x1x1xbf16>) outs(%8 : tensor<1x16x1x256xbf16>) {
    ^bb0(%in: bf16, %in_33: bf16, %out: bf16):
      %54 = arith.addf %in, %in_33 : bf16
      linalg.yield %54 : bf16
    } -> tensor<1x16x1x256xbf16>
    %30 = tensor.empty() : tensor<1x16x1x256xf32>
    %31 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%29 : tensor<1x16x1x256xbf16>) outs(%30 : tensor<1x16x1x256xf32>) {
    ^bb0(%in: bf16, %out: f32):
      %54 = arith.extf %in : bf16 to f32
      linalg.yield %54 : f32
    } -> tensor<1x16x1x256xf32>
    ...
    %36:2 = linalg.generic {indexing_maps = [#map3, #map8, #map8], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%31 : tensor<1x16x1x256xf32>) outs(%35, %33 : tensor<1x16x1xf32>, tensor<1x16x1xi64>) {
    ^bb0(%in: f32, %out: f32, %out_33: i64):
      ...
      %58 = arith.select %57, %55, %out_33 : i64
    ...
    %37 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%31, %expanded_24 : tensor<1x16x1x256xf32>, tensor<1x16x1x1xf32>) outs(%30 : tensor<1x16x1x256xf32>) {
    ^bb0(%in: f32, %in_33: f32, %out: f32):
      %54 = arith.subf %in, %in_33 : f32
      linalg.yield %54 : f32
    } -> tensor<1x16x1x256xf32>
    %38 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%37 : tensor<1x16x1x256xf32>) outs(%30 : tensor<1x16x1x256xf32>) {
    ^bb0(%in: f32, %out: f32):
      %54 = math.exp %in : f32
      linalg.yield %54 : f32
    } -> tensor<1x16x1x256xf32>
    ...
    %41 = linalg.generic {indexing_maps = [#map3, #map7], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%38 : tensor<1x16x1x256xf32>) outs(%40 : tensor<1x16x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %54 = arith.addf %in, %out : f32
      linalg.yield %54 : f32
    } -> tensor<1x16x1x1xf32>
    %42 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%38, %41 : tensor<1x16x1x256xf32>, tensor<1x16x1x1xf32>) outs(%30 : tensor<1x16x1x256xf32>) {
    ^bb0(%in: f32, %in_33: f32, %out: f32):
      %54 = arith.divf %in, %in_33 : f32
      linalg.yield %54 : f32
    } -> tensor<1x16x1x256xf32>
    %43 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%42 : tensor<1x16x1x256xf32>) outs(%8 : tensor<1x16x1x256xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %54 = arith.truncf %in : f32 to bf16
      linalg.yield %54 : bf16
    } -> tensor<1x16x1x256xbf16>
    %collapsed_25 = tensor.collapse_shape %43 [[0, 1, 2], [3]] : tensor<1x16x1x256xbf16> into tensor<16x256xbf16>
    %44 = linalg.generic {indexing_maps = [#map5, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_25 : tensor<16x256xbf16>) outs(%8 : tensor<1x16x1x256xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x1x256xbf16>
    %collapsed_26 = tensor.collapse_shape %concat_17 [[0, 1], [2], [3]] : tensor<1x16x256x256xbf16> into tensor<16x256x256xbf16>
    %45 = linalg.generic {indexing_maps = [#map6, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_26 : tensor<16x256x256xbf16>) outs(%22 : tensor<1x16x256x256xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x256x256xbf16>
    %collapsed_27 = tensor.collapse_shape %44 [[0, 1], [2], [3]] : tensor<1x16x1x256xbf16> into tensor<16x1x256xbf16>
    %collapsed_28 = tensor.collapse_shape %45 [[0, 1], [2], [3]] : tensor<1x16x256x256xbf16> into tensor<16x256x256xbf16>
    %46 = linalg.batch_matmul ins(%collapsed_27, %collapsed_28 : tensor<16x1x256xbf16>, tensor<16x256x256xbf16>) outs(%26 : tensor<16x1x256xbf16>) -> tensor<16x1x256xbf16>
    %expanded_29 = tensor.expand_shape %46 [[0, 1], [2], [3]] output_shape [1, 16, 1, 256] : tensor<16x1x256xbf16> into tensor<1x16x1x256xbf16>
    %47 = tensor.empty() : tensor<1x1x16x256xbf16>
    %transposed_30 = linalg.transpose ins(%expanded_29 : tensor<1x16x1x256xbf16>) outs(%47 : tensor<1x1x16x256xbf16>) permutation = [0, 2, 1, 3] 
    %48 = tensor.empty() : tensor<4096x3072xbf16>
    %transposed_31 = linalg.transpose ins(%arg9 : tensor<3072x4096xbf16>) outs(%48 : tensor<4096x3072xbf16>) permutation = [1, 0] 
    %collapsed_32 = tensor.collapse_shape %transposed_30 [[0, 1, 2, 3]] : tensor<1x1x16x256xbf16> into tensor<4096xbf16>
    %49 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%collapsed_32 : tensor<4096xbf16>) outs(%5 : tensor<1x1x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x1x4096xbf16>
    %50 = tensor.empty() : tensor<1x4096x3072xbf16>
    %51 = linalg.generic {indexing_maps = [#map2, #map1], iterator_types = ["parallel", "parallel", "parallel"]} ins(%transposed_31 : tensor<4096x3072xbf16>) outs(%50 : tensor<1x4096x3072xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x4096x3072xbf16>
    %52 = linalg.fill ins(%cst : bf16) outs(%1 : tensor<1x1x3072xbf16>) -> tensor<1x1x3072xbf16>
    %53 = linalg.batch_matmul ins(%49, %51 : tensor<1x1x4096xbf16>, tensor<1x4096x3072xbf16>) outs(%52 : tensor<1x1x3072xbf16>) -> tensor<1x1x3072xbf16>
    return %53 : tensor<1x1x3072xbf16>
  }
}
