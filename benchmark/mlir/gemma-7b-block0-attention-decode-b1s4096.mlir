...
module {
  func.func @main(%arg0: tensor<1x1x3072xbf16>, %arg1: tensor<1x1x256xbf16>, %arg2: tensor<1x1x256xbf16>, %arg3: tensor<1x16x1x1xbf16>, %arg4: tensor<1x16x4095x256xbf16>, %arg5: tensor<1x16x4095x256xbf16>, %arg6: tensor<4096x3072xbf16>, %arg7: tensor<4096x3072xbf16>, %arg8: tensor<4096x3072xbf16>, %arg9: tensor<3072x4096xbf16>) -> tensor<1x1x3072xbf16> {
    ...
    %concat_16 = tensor.concat dim(2) %arg4, %21 : (tensor<1x16x4095x256xbf16>, tensor<1x16x1x256xbf16>) -> tensor<1x16x4096x256xbf16>
    %concat_17 = tensor.concat dim(2) %arg5, %transposed_9 : (tensor<1x16x4095x256xbf16>, tensor<1x16x1x256xbf16>) -> tensor<1x16x4096x256xbf16>
    %22 = tensor.empty() : tensor<1x16x256x4096xbf16>
    %transposed_18 = linalg.transpose ins(%concat_16 : tensor<1x16x4096x256xbf16>) outs(%22 : tensor<1x16x256x4096xbf16>) permutation = [0, 1, 3, 2] 
    ...
    %collapsed_20 = tensor.collapse_shape %transposed_18 [[0, 1], [2], [3]] : tensor<1x16x256x4096xbf16> into tensor<16x256x4096xbf16>
    %24 = linalg.generic {indexing_maps = [#map6, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_20 : tensor<16x256x4096xbf16>) outs(%22 : tensor<1x16x256x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x256x4096xbf16>
    %collapsed_21 = tensor.collapse_shape %23 [[0, 1], [2], [3]] : tensor<1x16x1x256xbf16> into tensor<16x1x256xbf16>
    %collapsed_22 = tensor.collapse_shape %24 [[0, 1], [2], [3]] : tensor<1x16x256x4096xbf16> into tensor<16x256x4096xbf16>
    %25 = tensor.empty() : tensor<16x1x4096xbf16>
    %26 = linalg.fill ins(%cst : bf16) outs(%25 : tensor<16x1x4096xbf16>) -> tensor<16x1x4096xbf16>
    %27 = linalg.batch_matmul ins(%collapsed_21, %collapsed_22 : tensor<16x1x256xbf16>, tensor<16x256x4096xbf16>) outs(%26 : tensor<16x1x4096xbf16>) -> tensor<16x1x4096xbf16>
    %expanded_23 = tensor.expand_shape %27 [[0, 1], [2], [3]] output_shape [1, 16, 1, 4096] : tensor<16x1x4096xbf16> into tensor<1x16x1x4096xbf16>
    %28 = tensor.empty() : tensor<1x16x1x4096xbf16>
    %29 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_23 : tensor<1x16x1x4096xbf16>) outs(%28 : tensor<1x16x1x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      %58 = arith.mulf %in, %cst_2 : bf16
      linalg.yield %58 : bf16
    } -> tensor<1x16x1x4096xbf16>
    %30 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%29, %arg3 : tensor<1x16x1x4096xbf16>, tensor<1x16x1x1xbf16>) outs(%28 : tensor<1x16x1x4096xbf16>) {
    ^bb0(%in: bf16, %in_33: bf16, %out: bf16):
      %58 = arith.addf %in, %in_33 : bf16
      linalg.yield %58 : bf16
    } -> tensor<1x16x1x4096xbf16>
    %31 = tensor.empty() : tensor<1x16x1x4096xf32>
    %32 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%30 : tensor<1x16x1x4096xbf16>) outs(%31 : tensor<1x16x1x4096xf32>) {
    ^bb0(%in: bf16, %out: f32):
      %58 = arith.extf %in : bf16 to f32
      linalg.yield %58 : f32
    } -> tensor<1x16x1x4096xf32>
    ...
    %37:2 = linalg.generic {indexing_maps = [#map3, #map8, #map8], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%32 : tensor<1x16x1x4096xf32>) outs(%36, %34 : tensor<1x16x1xf32>, tensor<1x16x1xi64>) {
    ...
    %38 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%32, %expanded_24 : tensor<1x16x1x4096xf32>, tensor<1x16x1x1xf32>) outs(%31 : tensor<1x16x1x4096xf32>) {
    ^bb0(%in: f32, %in_33: f32, %out: f32):
      %58 = arith.subf %in, %in_33 : f32
      linalg.yield %58 : f32
    } -> tensor<1x16x1x4096xf32>
    %39 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%38 : tensor<1x16x1x4096xf32>) outs(%31 : tensor<1x16x1x4096xf32>) {
    ^bb0(%in: f32, %out: f32):
      %58 = math.exp %in : f32
      linalg.yield %58 : f32
    } -> tensor<1x16x1x4096xf32>
    ...
    %42 = linalg.generic {indexing_maps = [#map3, #map7], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%39 : tensor<1x16x1x4096xf32>) outs(%41 : tensor<1x16x1x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      %58 = arith.addf %in, %out : f32
      linalg.yield %58 : f32
    } -> tensor<1x16x1x1xf32>
    %43 = linalg.generic {indexing_maps = [#map3, #map7, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%39, %42 : tensor<1x16x1x4096xf32>, tensor<1x16x1x1xf32>) outs(%31 : tensor<1x16x1x4096xf32>) {
    ^bb0(%in: f32, %in_33: f32, %out: f32):
      %58 = arith.divf %in, %in_33 : f32
      linalg.yield %58 : f32
    } -> tensor<1x16x1x4096xf32>
    %44 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%43 : tensor<1x16x1x4096xf32>) outs(%28 : tensor<1x16x1x4096xbf16>) {
    ^bb0(%in: f32, %out: bf16):
      %58 = arith.truncf %in : f32 to bf16
      linalg.yield %58 : bf16
    } -> tensor<1x16x1x4096xbf16>
    %collapsed_25 = tensor.collapse_shape %44 [[0, 1, 2], [3]] : tensor<1x16x1x4096xbf16> into tensor<16x4096xbf16>
    %45 = linalg.generic {indexing_maps = [#map5, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_25 : tensor<16x4096xbf16>) outs(%28 : tensor<1x16x1x4096xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x1x4096xbf16>
    %46 = tensor.empty() : tensor<1x16x4096x256xbf16>
    %collapsed_26 = tensor.collapse_shape %concat_17 [[0, 1], [2], [3]] : tensor<1x16x4096x256xbf16> into tensor<16x4096x256xbf16>
    %47 = linalg.generic {indexing_maps = [#map6, #map3], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%collapsed_26 : tensor<16x4096x256xbf16>) outs(%46 : tensor<1x16x4096x256xbf16>) {
    ^bb0(%in: bf16, %out: bf16):
      linalg.yield %in : bf16
    } -> tensor<1x16x4096x256xbf16>
    %collapsed_27 = tensor.collapse_shape %45 [[0, 1], [2], [3]] : tensor<1x16x1x4096xbf16> into tensor<16x1x4096xbf16>
    %collapsed_28 = tensor.collapse_shape %47 [[0, 1], [2], [3]] : tensor<1x16x4096x256xbf16> into tensor<16x4096x256xbf16>
    %48 = tensor.empty() : tensor<16x1x256xbf16>
    %49 = linalg.fill ins(%cst : bf16) outs(%48 : tensor<16x1x256xbf16>) -> tensor<16x1x256xbf16>
    %50 = linalg.batch_matmul ins(%collapsed_27, %collapsed_28 : tensor<16x1x4096xbf16>, tensor<16x4096x256xbf16>) outs(%49 : tensor<16x1x256xbf16>) -> tensor<16x1x256xbf16>
