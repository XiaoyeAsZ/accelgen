...
class GPT2Attention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, past_key_values=None, cache_position=None):
        # 处理 KV cache
        if KV_CACHE and past_key_values is not None:
...
print(f"KV Cache enabled: {KV_CACHE}")
...
if KV_CACHE:
    ...
    dummy_input = (initial_input,)
    output_file = "./mlir/gpt2_kv_open.mlir"
else:
    # no cache
    print("\n=== Testing without KV Cache ===")
    dummy_input = (torch.randn(1, 16, 768, dtype=torch.bfloat16),)
    output, _ = model(dummy_input[0])
    print(f"Output shape: {output.shape}")
    output_file = "./mlir/gpt2_kv_close.mlir"
